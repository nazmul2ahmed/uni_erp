...
export const tenants = control.table("tenants", {
  ...
  // Canonical Owner pointer — per 05 §75a (Decision TEN-001).
  // INV-OWN-001: MUST NOT be null while status = ACTIVE (enforced in
  // application layer inside CreateTenantUseCase / TransferOwnershipUseCase,
  // not by a DB CHECK, since it is nullable during the brief provisioning
  // window before the first membership is created).
