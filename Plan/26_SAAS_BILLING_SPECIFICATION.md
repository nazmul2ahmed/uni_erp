...
# 6. Feature Resolution — Concrete Algorithm (REVISED — Phase 0.5 Reconciliation)

Completes `05` §71's abstract chain. **`moduleState` (previously step 4)
is removed per Decision BIL-007** — see rationale below.
...
resolveFeature(tenantContext, featureKey) -> boolean:
  ...
  4. userPermission = does the current actor's role include the
     relevant resource.action permission? (04 §34 — orthogonal to
     billing entirely, RBAC not plan-driven)
  5. return ALL of steps 1-4 pass
```

**Rationale for removing `moduleState` (Decision BIL-007):** none of the
Optional Modules (`14`–`18`) have a provisioning/runtime lifecycle
distinct from their `tenant_features` toggle — enabling a module is a
pure configuration change with immediate effect, not a multi-step
process. A separate `moduleState` layer added redundant checking with
no corresponding real state to check, and risked divergence bugs where
`tenant_features.rental = true` but `moduleState.rental` disagreed. If a
future Optional Module genuinely requires multi-step provisioning (e.g.
an AI-driven module needing tenant-specific model setup), it receives
its own dedicated state machine, following the pattern already
established for Dedicated Tenant Provisioning (`05` §51) — not a
platform-wide generic layer applied to every module regardless of need.
...
### Decision BIL-007 (NEW — Phase 0.5 Reconciliation)
`moduleState` is removed from the Feature Resolution algorithm (§6) —
`tenant_features` is the sole source of truth for Optional Module
enablement. Resolves `26` §14 Q2 (Phase 0.5 Reconciliation, human-approved).
...
2. [RESOLVED — see Decision BIL-007, §6] Is `moduleState` (previously
   §6 step 4) a genuinely necessary fourth layer, or is it redundant
   with `tenant_features`? Collapsed out of the resolution algorithm.
