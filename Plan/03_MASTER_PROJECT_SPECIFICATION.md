...
**Status update (NEW — Phase 0.5 Reconciliation):** the `REQ-CORE-001`
/ `REQ-SALES-001`-style traceability format originally proposed below
was never adopted in practice. The actual, working traceability
mechanism across `07`–`29` is the document-local `Decision
<PREFIX>-###` pattern (e.g. `DOM-001`, `SYNC-002`, `TEN-001`), each
scoped to its owning document with an explicit citation trail back to
prior documents. This section is retained for historical context only;
`REQ-*` tags are formally retired and should not be introduced in future
documents.
