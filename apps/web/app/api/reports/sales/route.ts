...
import { getSalesReport } from "@/lib/use-cases/reports";

const dateFilterSchema = z.object({ dateFrom: z.string().date().optional(), dateTo: z.string().date().optional() }).refine((value) => !value.dateFrom || !value.dateTo || value.dateFrom <= value.dateTo, "dateFrom must be before dateTo");
...
export async function GET(req: NextRequest) {
  return apiHandler(async () => {
    ...
    return getSalesReport(ctx, parsed.data);
  })();
}